  ...
  <div class="container mx-auto bg-white shadow-lg rounded-lg p-6">
    ...
    <div class="flex justify-between mb-4">
        <form action="{{ route('requested_shifts.index') }}" method="GET">
            <input type="hidden" name="date" value="{{ $currentWeek['start']->format('Y-m-d') }}">
        ...
        <form action="{{ route('requested_shifts.index') }}" method="GET">
            <input type="hidden" name="date" value="{{ $currentWeek['end']->addDay()->format('Y-m-d') }}">
    ...
    <table class="table-auto w-full border-collapse border border-blue-300">
      ...
      <tbody>
        ...
        <tr>
          <td class="border border-blue-300 px-4 py-2 bg-blue-100 text-blue-900">{{ $requested_shift->name }}</td>
          <td class="shift-cell border border-blue-300 px-4 py-2 bg-white hover:bg-blue-100 cursor-pointer">{{ $requested_shift->start->format('H:i') }}</td>
          <!-- 他の曜日のセルも同様に表示 -->
        </tr>
        @endforeach
      </tbody>
